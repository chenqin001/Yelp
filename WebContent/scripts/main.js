(function() {
  ...
  function init() {
    ...
    //validateSession();
    onSessionValid({"user_id":"1111","name":"","status":"OK"});
  }
  ...
  function initGeoLocation() {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        onPositionUpdated,
        onLoadPositionFailed, { //fail的话就，时间间隔60秒
          maximumAge: 60000
        });
  ...
  function activeBtn(btnId) {
    ...
    // 先deactivate all navigation buttons
 ...
 // make AJAX call
    ...
    ajax(method, url, req,
      ...
      function(res) {
        ...
        if (result.status === 'OK' || result.result === 'SUCCESS') {
          ...
          favIcon.className = favorite ? 'fa fa-heart' : 'far fa-heart';
        }
      });
  }
  ...
  function addItem(itemList, item) {
    ...
    favLink.appendChild($create('i', {
      id: 'fav-icon-' + item_id,
      className: item.favorite ? 'fa fa-heart' : 'far fa-heart'
    }));
